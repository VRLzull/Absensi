...
const FaceRegistration = () => {
  ...
  const handleDeleteFaces = async (employee) => {
    const res = await confirmAction(
      'Hapus Foto Wajah?',
      `Apakah Anda yakin ingin menghapus semua foto wajah ${employee.full_name}?`
    );
    if (!res.isConfirmed) return;
    try {
      showLoading('Menghapus...', 'Memproses penghapusan data wajah');
      showSnackbar('Foto wajah berhasil dihapus', 'success');
      await showSuccess('Foto wajah berhasil dihapus');
      loadEmployees();
    } catch (error) {
      await showError('Gagal menghapus foto wajah', error.message);
    } finally {
