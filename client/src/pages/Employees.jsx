...
import { confirmAction, showLoading, showSuccess, showError } from '../utils/popup';
...
const Employees = () => {
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text('Data Pegawai/Siswa', 14, 15);
    
    const tableColumn = ["ID", "Nama", "Email", "Departemen", "Jabatan", "Telepon", "Gender"];
  ...
  const handleDelete = async (employee) => {
    const res = await confirmAction(
      'Hapus Pegawai?',
      `Apakah Anda yakin ingin menghapus ${employee.full_name}?`
    );
    if (!res.isConfirmed) return;
    try {
      showLoading('Menghapus...', 'Memproses penghapusan');
      const token = localStorage.getItem('token');
      if (!token) {
        await showError('Gagal', 'Token autentikasi tidak ditemukan');
        return;
      }
      ...
      if (!response.ok || !result.success) {
        await showError('Gagal menghapus pegawai', result.error || 'Terjadi kesalahan');
        return;
      }
      await showSuccess('Pegawai berhasil dihapus');
      loadEmployees();
    } catch (error) {
      await showError('Gagal menghapus pegawai', error.message);
    } finally {
