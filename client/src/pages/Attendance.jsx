...
const Attendance = () => {
  ...
  const handleSubmitFeedback = async () => {
    try {
      ...
      setFeedbackForm({ category: 'Absensi', title: '', description: '' });
    } catch (error) {
  ...
  const handleExport = async () => {
    try {
      ...
      if (data.length === 0) {
        await showError('Tidak ada data', 'Tidak ada data absensi untuk diekspor berdasarkan filter yang dipilih');
        return;
      }
      ...
      await showError('Gagal mengekspor data', error.message);
    } finally {
  ...
  const handleDeleteAttendance = async () => {
    ...
    try {
      ...
      await showError('Gagal menghapus data absensi', error.message);
    } finally {
      setDeletingId(null);
    ...
    <PageTransition>
      <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={user?.role === 'student' ? 4 : 3}>
            <Card sx={{ 
              bgcolor: '#10b981', 
              color: '#ffffff',
              borderRadius: 2,
              ...
              <CardContent sx={{ p: 3, textAlign: 'center' }}>
                <Typography variant="h4" sx={{ fontWeight: 800, mb: 1 }}>
                  {user?.role === 'student' ? filteredAttendance.filter(a => a.status === 'present').length : stats.present}
                </Typography>
                <Typography variant="body2" sx={{ opacity: 0.9 }}>
                  Hadir
                </Typography>
          ...
          <Grid item xs={12} sm={6} md={user?.role === 'student' ? 4 : 3}>
            <Card sx={{ 
              ...
              <CardContent sx={{ p: 3, textAlign: 'center' }}>
                <Typography variant="h4" sx={{ fontWeight: 800, mb: 1 }}>
                  {user?.role === 'student' ? filteredAttendance.filter(a => a.status === 'late').length : stats.late}
                </Typography>
                <Typography variant="body2" sx={{ opacity: 0.9 }}>
                  Terlambat
                </Typography>
          ...
          <Grid item xs={12} sm={6} md={user?.role === 'student' ? 4 : 3}>
            <Card sx={{ 
              ...
              <CardContent sx={{ p: 3, textAlign: 'center' }}>
                <Typography variant="h4" sx={{ fontWeight: 800, mb: 1 }}>
                  {user?.role === 'student' ? filteredAttendance.filter(a => a.status === 'absent').length : stats.absent}
                </Typography>
                <Typography variant="body2" sx={{ opacity: 0.9 }}>
                  Tidak Hadir
                </Typography>
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ p: 4 }}>
            <Grid container spacing={3} alignItems="center">
              ...
              <Grid item xs={12} md={user?.role === 'student' ? 6 : (filterRange === 'date' || filterRange === 'month' ? 3 : 4)}>
                {user?.role !== 'student' ? (
                  <TextField
                    fullWidth
                  ...
                  <Box sx={{ p: 2, bgcolor: '#f1f5f9', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Dialog open={openFeedbackDialog} onClose={() => setOpenFeedbackDialog(false)} maxWidth="sm" fullWidth>
          ...
          <DialogActions>
            <Button onClick={() => setOpenFeedbackDialog(false)}>Batal</Button>
            <Button onClick={handleSubmitFeedback} variant="contained">Kirim</Button>
          </DialogActions>
        </Dialog>
        {/* Export Dialog */}
